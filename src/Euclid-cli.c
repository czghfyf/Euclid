...
//    }
	...
	send(sock_cli, fixc_intent_cli->mem_addr,
	     fixc_intent_cli->data_pkg_capacity, 0);
	printf(">>> send >>> intent.\n");
...
//                printf("serv_resp -> command_length is %d\n", serv_resp -> command_length);
	...
	if (p_join_node_host != NULL) {
		...
		*((int *)(mem_addr + 22)) = p_join_node_port;
		...
		send(sock_cli, join_cluster_command->mem_addr,
		     join_cluster_command->data_pkg_capacity, 0);
		printf(">>> send >>> go to join[%s:%d] a cluster.\n",
		       join_cluster_command->command,
		       *((int *)(join_cluster_command->command + 16)));
